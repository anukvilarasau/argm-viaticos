...
export function WeeklyPlannerBoard({
  ...
  return (
    <View className="rounded-[32px] border border-white/40 bg-white/74 p-5 shadow-sm">
      <View className="mb-5 flex-row items-center justify-between">
        <View>
          <Text className="text-sm font-medium uppercase tracking-[1.6px] text-text/65">Calendario dinámico</Text>
          <Text className="mt-2 text-[28px] font-semibold capitalize text-text">{formatMonthShort(selectedDate)}</Text>
        </View>
          ...
          {weekDates.map((date) => {
            ...
            return (
              <Pressable key={date} className="flex-1" onPress={() => onSelectDate(date)}>
                <View className={`mb-2 rounded-[20px] px-2 py-3 ${active ? "bg-[#ECE4FF]" : "bg-white/60"}`}>
                  <Text className="text-center text-xs font-medium uppercase text-text/55">{formatWeekdayShort(date)}</Text>
                  <Text className={`mt-1 text-center text-[24px] font-semibold ${active ? "text-accent" : "text-text"}`}>
                    {formatDayNumber(date)}
                  </Text>
                </View>

                <View className="relative overflow-hidden rounded-[24px] border border-black/5 bg-white/55" style={{ height: BOARD_HEIGHT }}>
                  {hours.slice(0, -1).map((hour) => (
                    <View
                      key={`${date}-${hour}`}
                  ...
                  {dayAgenda.timeline.map((event) => {
                    ...
                    return (
                      <View
                        key={event.id}
                        className={`absolute left-1 right-1 rounded-[16px] px-2 py-2 shadow-sm ${categoryTone[event.category]}`}
                        style={{ minHeight: 54, top }}
                      >
                  ...
                  {active && !dayAgenda.timeline.length ? (
                    <View className="absolute inset-x-2 top-40 items-center rounded-[18px] bg-white/90 px-3 py-6">
                      <Feather color="#17181C" name="plus" size={26} />
                    </View>
                  ) : null}
                </View>
              </Pressable>
